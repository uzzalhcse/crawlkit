...
func handleDynamicCrawl(app *crawler.Crawler) {
	...
	app.ProductDetailSelector = crawler.ProductDetailSelector{
		...
		Category: &crawler.CategorySelector{
			DataSelector: &crawler.DataSelector{
				Query: "div#container p.topicpath.pc_only a",
			},
		},
		Description: "",
	}
	app.Collection(constant.PRODUCT_DETAILS).IsDynamicPage(false).SetCrawlLimit(40).CrawlPageDetail(constant.PRODUCTS)
}
